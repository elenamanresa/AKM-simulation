...
data  = data.table(reshape2::melt(network,c('i','t'))) 
data2 = data.table(reshape2::melt(spellcount,c('i','t')))
...
Compute: - mean firm size, in the crossection, expect something like 10. - mean number of movers per firm in total in our panel (a worker that moved from firm i to j is counted as mover in firm i as well as in firm j).
...
We use this generated data to create the event study plot from Card-Heining-Kline:
...
-   line 2 is just `data[,mean(resid(mod) + coef(mod)[2]*D1),by=i]`
-   line 3 is just `data[,mean(resid(mod) + coef(mod)[3]*D2),by=j]`
...
We now compute the variance decomposition and compare it with its unfeasible counterpart, using alpha and psi, instead of $\hat{\alpha}$ and $\hat{\psi}$.
...
## Regularization with k-means: 
Kgroup = 6
rr <- data[,.(mean_lw =mean(lw),stdev_lw = sqrt(var(lw))),by=fid]
Nschools = rr[,.N]
data_matrix <- matrix(c(rr$mean_lw,rr$stdev_lw),nrow=Nfirm_con,ncol=2,byrow=FALSE)
kmeans_result <- kmeans(data_matrix, centers = Kgroup, nstart=100)
rr[,cl_f:=kmeans_result$cluster]
data_final2 <-merge(data, rr, by = "fid", all.x = TRUE)
#data <- data_final2
data_final2[,k:=cl_f]
nk = Kgroup

data_final2[,mean_fid:=mean(lw),by=fid]
data_final2[,mean_cl:=mean(lw),by=k]

var_exp = var(data_final2$mean_cl)/var(data_final2$mean_fid)
print("variance explained by clusters")
print(var_exp)

data_final2[,mean_fid:=NULL]
data_final2[,mean_cl:=NULL]

```

## Regularization by clusters

We now use k-means to define clusters of firms that approximate the firm-specific unobserved heterogeneity.

```{r}
# We now compute the variance decomposition based on this cluster-level firm effects:


data_final2[, fid:=k]
dd=guimaraesPortugal(data_final2)
```

The variance decomposition based on clusters is: $(var(\hat{\alpha}),var(\hat{\psi}(\hat{k}_i)),cov(\hat{\alpha},\hat{\psi}(\hat{k}_i)))$:

```{r}
c(var(data_final2$alpha_hat),var(data_final2$psi_hat),cov(data_final2$alpha_hat,data_final2$psi_hat))
```
